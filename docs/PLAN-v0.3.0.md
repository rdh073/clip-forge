...
| i      | `bin/cf-ffmpeg render` (hook overlay + progress bar + aspect + sidecar plumbing) · `bin/cf-caption-burn` (emoji + highlight + sidecar flags) · `bin/lib/overlay-builder.mjs` + `.test.mjs` · `bin/lib/srt-vtt.mjs` + `.test.mjs` · `agents/caption-stylist.md` (hook_span output) · `skills/render/SKILL.md` + `skills/caption/SKILL.md` · `templates/captions/{Submagic-Pop,Beast}.json` (hook_overlay block) · `schemas/render_report.v1.json` (target_aspect / overlays / sidecars fields) · `tests/integration/overlay.test.mjs` | shipped at commit `7ed2f4f` (2026-05-21) |
