...
- 2026-05-21 — pillar 3 (brand kit / custom assets) shipped at SHA `309cd03`. Schema `schemas/brand-kit.v1.json` v1 finalised: `{version, name, assets: {logo, endcard, lower_third}}`. Per-project (`./uploads/<slug>/brand-kit.json`) wins entirely over global (`~/.clip-forge/brand-kit.json`); same precedence rule as voices.json from pillar 2. File-size limits enforced at LOAD time (logo / lower-third PNG ≤ 2 MB, endcard PNG ≤ 2 MB / MP4 ≤ 3 MB) — oversized assets are skipped with `brand_kit_asset_oversize` before the filter graph is built, keeping ffmpeg memory bounded. `edit.json.brand_kit` (inline) > `watermark.brand_kit_ref` (file pointer) > legacy `watermark: "<path>"` (string) > project/global file. SVG support is best-effort: `librsvg_not_available` warning + skip when ffmpeg lacks librsvg; PNG assets in same kit still render. Color tokens `$brand.colors.primary` / `$brand.colors.accent` documented as a DESIGN HOOK only — token expansion deferred to v0.5.0; `$brand.logo` is the only substitution wired this round (see `agents/caption-stylist.md`). New `bin/cf-brand-kit` dispatcher with `add | list | set-default | remove` subcommands; new `/clip-forge:brand-kit` skill. Backward-compat regression test confirms legacy `"watermark": "<path>"` still produces a bottom-right logo overlay.
- 2026-05-21 — pillar 4 (cf-edit + prompt-driven re-edit + Anthropic translate completion) shipped at SHA `fb0499c`. Three deliverables bundled: (A) content-hash diff + partial re-render via `bin/lib/render-manifest.mjs` + `bin/cf-edit`; (B) LLM prompt mode via `bin/lib/llm.mjs` (groq + anthropic adapters) + `bin/lib/edit-patch.mjs` 3-layer validation (schema + whitelist + dry-run preview); (C) real-network Anthropic translate adapter completing the stub left by pillar 2. Schema-path alignment: the brief's initial whitelist (`/hook/text`, `/hook/duration_ms`, `/aspect_target`, `/captions/style`) did NOT match the actual edit.json shape from pillar (i). Whitelist amended to: `/hook_overlay/text`, `/hook_overlay/end_ms`, `/hook_overlay/position`, `/progress_bar/*`, `/target_aspect`, `/cuts`, `/brand_kit`, `/watermark`. Caption-style edits (`/captions/style`) deliberately EXCLUDED — captions style lives in `captions.json`, a sibling file not editable via `edit.json` patches; the renderer reads `captions` as a path to the `.ass` file, not a style object. Forbidden paths (`/crop_path`, `/audio_source`, `/clip_id`, `/source`, `/output`, `/version`) reject with `rejected_reason: "off_whitelist"`. New `schemas/render_manifest.v1.json` formalises the per-clip input-hash record + the pillar-2 `ai_costs` block in one file (`additionalProperties: true` at the top so future pillars can extend additively). Atomic manifest write: write `<path>.tmp.<pid>.<ts>`, `fsync`, `rename` — kill -9 mid-write leaves either the previous manifest intact OR no manifest, never partial JSON. Pillar 2's `ai_costs` block is preserved byte-for-byte modulo additive breakdown keys (`groq_llm`, `anthropic_llm`, `anthropic_translate`); integration test E7 + composition gate verify this. LLM provider precedence: `CF_LLM_PROVIDER` override → `GROQ_API_KEY` → `ANTHROPIC_API_KEY` → `no_llm_provider` graceful degrade. Anthropic translate completion uses claude-haiku-4-5-20251001 with the same `reattachTiming()` helper as the Groq path; live test gated behind `CF_TRANSLATE_REAL_E2E=1` + `ANTHROPIC_API_KEY`. `render_report.v1` extended additively with `rerender: {reason, stale_keys, manifest_path}` and `llm: {patch_applied, provider_used, retry_count, rejected_reason, cost_usd}` — existing required-field list unchanged.
...
- 2026-05-21 — pillar 5 (AI B-roll + avatar stingers) shipped at SHA `8964123` (+ reviewer YAML-frontmatter fix `63b3200`). Two new skills (`/clip-forge:broll-ai`, `/clip-forge:avatar`) + two new dispatchers (`bin/cf-broll-ai`, `bin/cf-avatar`) + five new BYO env vars (`FAL_API_KEY`, `GEMINI_API_KEY`, `REPLICATE_API_TOKEN`, `HEYGEN_API_KEY`, `DID_API_KEY` — all optional, graceful degrade). Two abstraction layers: `bin/lib/visual.mjs` with adapters under `bin/lib/visual/{fal,nanobanana,replicate}.mjs`; `bin/lib/avatar.mjs` with adapters under `bin/lib/avatar/{heygen,did,fal_lip}.mjs`. The moat-anchor invariant — AI never operates on the creator's primary footage — is enforced at THREE layers: (1) `broll.json` segments carry `is_primary: bool`; (2) dispatchers refuse `is_primary: true`; (3) renderer dies with `ai_primary_refusal:<slot>` if any AI asset declares `is_primary: true` (defense in depth, verified by composition test). Auto-detect heuristic uses the existing `crop_path.json.stats.framesWithFace / framesProcessed` ratio (threshold 0.5) — conservative on purpose; no new face-detection pass added. Avatar consent state machine via `bin/lib/consent-log.mjs`: gate 1 one-time per machine (bilingual EN+ID prompt, `CF_AVATAR_CONSENT=1` bypass for CI); gate 2 per-photo sha256 cache (re-used photos skip the prompt + bump `use_count`). Consent log path `~/.clip-forge/.consent-log` with atomic write semantics mirroring pillar 4 manifest pattern (write `.tmp.<pid>.<ts>` → fsync → rename). Hard caps: avatar stinger ≤ 5000 ms (refused at dispatcher AND adapter layer); AI B-roll cutaway ≤ 3000 ms. Pillar-2 `ai_costs.history` + pillar-4 `clips` block survive byte-for-byte when cf-broll-ai / cf-avatar write the manifest — composition test pre-seeds `elevenlabs_tts: 0.30 + groq_llm: 0.04` and verifies preservation post-rewrite. `render_report.v1` extended additively with `stingers: {count, types, providers_used, consent_verified, total_duration_ms}` and `broll_ai: {count, providers_used, gaps_filled, stylized_segments}` — existing required-field list unchanged. Mock injection: `CF_VISUAL_MOCK`, `CF_AVATAR_MOCK`, `CF_CONSENT_MOCK=auto-yes|auto-no` (every paid path mocked; CI green without any provider keys). Tests: 45 unit + 25 integration; baseline 336/324 → 406/394 with 12 skipped + 0 failed; zero regressions. LOC actual: ~1140 code + ~700 test (under the 1300 cap; pattern from pillar 4 dispatcher+adapter shape held).
- 2026-05-21 — pillar 6 (speaker-aware reframe + split-screen) shipped at SHA `set after rebase`. New pure-logic library `bin/lib/speaker-timeline.mjs` walks the per-word `speaker` field from `transcript.json` and emits 1500 ms-windowed multi-speaker intervals (Deepgram diarizes natively; Whisper diarize via sherpa-onnx VAD remains opt-in via `--diarize sherpa`, **deferred to v0.5.0**). Warnings: `no_speaker_labels`, `single_speaker`, `diarize_low_confidence`. `bin/cf-reframe` gains `--speaker-route auto|none`; default `auto` weaves `split_screen` samples into the output crop path when ≥ 2 distinct speakers are present; `none` forces v0.2.0 single-face behaviour. `crop_path.json` schema v2 → v3 **additive**: v3 readers consume v2 inputs unchanged; v2 readers receiving v3 crop_paths without `split_screen` samples render identically to v2 (top-level `speaker_timeline` block is ignored). Split-screen sample shape carries per-speaker `{speaker_id, cx, cy, scale}` instead of v2's single-face `{cx, cy, scale}`. New builders in `bin/lib/crop-expression-builder.mjs`: `chooseSplitAxis` (9:16/4:5 → vstack, 1:1/16:9 → hstack), `buildSplitScreenFilter`, `buildSplitScreenScript`, `groupSplitScreenWindows`, `summarizeSplitScreenSamples`. **Identity stability invariant (S3):** speaker order in the filter graph is forced ascending by `speaker_id` so `speaker_id 0` always occupies the LEFT (hstack) or TOP (vstack) panel within a window — no mid-window flips. `bin/cf-ffmpeg render` consumes v3 samples via a filter-script path that overlays per-window stacks onto the single-face base with `enable='between(t, t0, t1)'`. Brand kit, hook overlay, captions, and progress bar compose downstream of the split-screen output. **Splice (tighten cuts) + split-screen combo:** when both are present the renderer emits a `split_screen_disabled_by_splice` warning and falls back to single-face — the combined filter graph is non-trivial and the combination is deferred to v0.5.0. `render_report.v1` extended additively with `split_screen: {windows_count, total_duration_ms, speakers_detected, stack_axis, warnings}`. README parity row "Speaker-aware reframe (split-screen)" flipped to ✅; new "Multi-speaker content" section documents aspect → stack-axis mapping. Tests: 10 unit (speaker-timeline) + 9 unit (crop-expression-builder split-screen) + 7 integration (split-screen) + 2 integration (pillar-6 composition); baseline 406/394 → 435/423 with 12 skipped + 0 failed; zero regressions on pillars 1-5 / v0.3.0 / v0.2.0. LOC actual: ~540 code + ~1030 test (under the 800 code cap; no API surface, no adapter pattern — matches the pillar 3 footprint).

## 11. v0.4.0 ship readiness — pillar 6 closes the release

All six picks from §2 have shipped on master. Each EXIT CRITERIA gate from §9 is verified below with the commit ref that satisfies it.

| # | §9 Gate | Status | Evidence |
|---|---|:-:|---|
| 1 | All 6 picks shipped on master | ✅ | `85a2b7e` (pillar 1) → `e109a97` (pillar 2) → `309cd03` (pillar 3) → `fb0499c` (pillar 4) → `8964123` (pillar 5) → `set after rebase` (pillar 6). Each pick has one feat commit + one doc-fill commit. |
| 2 | 100 % mock-path test coverage for paid features | ✅ | `CF_TTS_MOCK` / `CF_TRANSLATE_MOCK` / `CF_VISUAL_MOCK` / `CF_AVATAR_MOCK` / `CF_CONSENT_MOCK` injection points all carry positive-evidence integration tests; baseline CI runs green without any API keys set. Pillar 6 has no paid path so no new mock injection added. |
| 3 | All v0.3.0 + v0.2.0 regression tests still passing | ✅ | Full suite at `set after rebase`: 435 / 423 / 12 skipped / 0 failed. v0.2.0 face-tracked success-path tests gated on Ultraface ONNX model presence (skipped in CI; locally green when `node bin/install-models.mjs` has run). |
| 4 | README updated with BYO-key section | ✅ | Shipped with pillar 2 (`e109a97`); pillar 5 (`8964123`) added the AI Tier-2 hard-constraints section above it; pillar 6 added the Multi-speaker content section after it. All sections sit ABOVE Install instructions. |
| 5 | End-to-end multi-lang dub demo | ✅ | Shipped with pillar 2 (`e109a97`) at `examples/dub-demo/`. |
| 6 | `claude plugin validate .` clean | ✅ | 0 errors / 0 warnings on tip-of-master after pillar 6 commit. Reviewer must re-verify INDEPENDENTLY (pillar 5 caught a YAML colon-space issue that the agent had missed). |
| 7 | CI lint: `.env` not tracked | ✅ | `git ls-files .env` returns empty; `.env.example` carries the template, `.env` is in `.gitignore`. |
