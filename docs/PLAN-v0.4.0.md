...
|---|---|:-:|---|
| 1 | All 6 picks shipped on master | ✅ | `85a2b7e` (pillar 1) → `e109a97` (pillar 2) → `309cd03` (pillar 3) → `fb0499c` (pillar 4) → `8964123` (pillar 5) → `d50d057` (pillar 6). Each pick has one feat commit + one doc-fill commit. |
...
| 3 | All v0.3.0 + v0.2.0 regression tests still passing | ✅ | Full suite at `d50d057`: 435 / 423 / 12 skipped / 0 failed. v0.2.0 face-tracked success-path tests gated on Ultraface ONNX model presence (skipped in CI; locally green when `node bin/install-models.mjs` has run). |
